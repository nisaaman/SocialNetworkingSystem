...
class se_user {
	...
	// OUTPUT: 
	function user_fields($tabs_only = 0, $tab_id = 0, $validate = 0, $signup = 0, $profile = 0) {
	  ...
	  while($tab_info = $database->database_fetch_assoc($tabs)) {
	    ...
	    if($tabs_only == 0) {
	      $field_count = 0;
	      $this->profile_fields = "";
	      $field_query = "SELECT * FROM se_fields WHERE field_tab_id='$tab_info[tab_id]'";
		   if($signup != 0) { $field_query .= " AND field_signup<>'0'"; } 
		   if($profile != 0) { $field_query .= " AND field_browsable<>'-1'"; } 
		   $field_query .= " ORDER BY field_order";
	...
	function user_message_send($to, $subject, $message, $convo_id = 0) {
	  ...
	  // TO USER IS NOT A FRIEND  HAS "MESSAGE FRIENDS ONLY" TURNED ON
	  ...
	  if($this->is_error == 0) {
	    ...
	    $num_inbox = $to_user->user_message_total(0, 0);
	...
	function user_create($signup_email, $signup_username, $signup_password, $signup_timezone, $signup_language) {
	  ...
	  // PRESET VARS
	  $signup_level_info = $database->database_fetch_assoc($database->database_query("SELECT level_id, level_profile_comments FROM se_levels WHERE level_default='1' LIMIT 1"));
	  $signup_date = time();
